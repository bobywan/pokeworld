...
import { STAGE_FR, TYPE_COLORS } from "@/lib/constants";
...
export function PokemonCard({ pokemon }: PokemonCardProps) {
  ...
  return (
    <div className="pokemon-card group relative overflow-hidden rounded-card bg-surface shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
      ...
      {/* Corps de la carte */}
      <div className="p-3">
        <h2 className="mb-2 text-sm font-bold leading-tight text-text-primary">{pokemon.name}</h2>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-baseline gap-1">
      ...
      <span className="truncate font-semibold text-text-secondary">{value}</span>
    </div>
  );
}
